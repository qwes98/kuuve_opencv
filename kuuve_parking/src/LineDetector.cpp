...
// ���� �ʱ��� ã��
int LaneDetect::find_L0_x(Mat binary_img, int img_height, int *framecount_L , int Right0_x, int offset)
{
	int l0_x = Right0_x;

	for (int i = offset; i < binary_img.cols-15; i++)
	{
...
int LaneDetect::find_next_R0_x(Mat binary_img, int img_height , int *framecount_R ,int Left0_x, int offset)
{
	int r0_x = Left0_x;

	for (int i = offset; i < binary_img.cols-17; i++)
	{
		if (binary_img.at<uchar>(img_height , binary_img.cols - i) == 255)
		{
			r0_x = binary_img.cols - i;
